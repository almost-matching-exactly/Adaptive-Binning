...
setup_ahb_mip = function(xi, fhati1, fhati0, fhat0, fhat1, x_test, z_test, 
                         alpha=1, beta=1, gamma1=1, gamma0=0, m=1, M=1e10){
...
AHB_MIP_match <- function(unit_id, test_df, fhat1, fhat0, alpha=1, beta=1, gamma1=1, gamma0=0, m=1, M=1e10,
                          preprocess = TRUE, ncandt=100, ncandc=100){
  
  i = unit_id
  test_covs = test_df[, 1:(ncol(test_df)-2)]
  
  if(preprocess){
    ord = order(abs(fhat1[i] - fhat1) * gamma1/sd(fhat1) + abs(fhat0[i] - fhat0) * gamma0/sd(fhat0))
    cands = c(ord[test_df$treated[ord]==1][1:ncandt], ord[test_df$treated[ord]==0][1:ncandc])
  }else{
    cands = (1:nrow(test_df))[-i]
  }
  
  mip_pars =  setup_ahb_mip(xi = as.numeric(test_covs[i, ]),
  ...
  mip_out$mg1 = make_mg(test_covs, mip_out$a, mip_out$b)
  mip_out$mg2 = which(mip_out$w > 1/M)
  return(mip_out)
}
