...
ggplot(simdata) + 
  ...
  geom_raster(aes(x=x1, y=x2, fill=y)) + 
...
```{r "sim 1", cache=T}
res = matching_sim(10, 200, 3, lambda=1, alpha=-2 )
...
# }
res = matching_sim(5, 200, 3, 
                   estimators = c('CEM', 'Nearest Neighbor', 'Full Matching', 'Greedy'))
res$estimator %<>% factor(levels = c('Greedy', 'Nearest Neighbor', 'CEM', 'Full Matching'))
...
res = matching_sim(10, 200, 3, HTE_dgp = HTE_dgp, y_dgp=y_dgp,lambda=1, alpha=-2)
res$estimator = factor(res$estimator, levels=c("MIP-Explain", "MIP-Predict", "MIQP-Variance", 
                                               "Greedy", "CEM", "Full Matching" , "Nearest Neighbor", 
                                               "Prognostic", "Mahalanobis"))
```
...
c(miqp_cates, miqp_bins) %<-% est_MIQP_variance(train_df, test_df, test_treated, n_test_treated, train_covs, 
                                                test_covs, n_train, p, lambda=3, alpha=0, m=2)
...
ge = abs(HTE_true - greedy_cates)
me = abs(HTE_true - miqp_cates)
me2 = abs(miqp_cates2 - HTE_true)
...
c(df, HTE) %<-% simulate_data(200, 3, y_dgp = y_dgp)
c(df, f, n, n_train, p,
    train_df, train_covs, train_control, train_treated,
    test_df, test_covs, test_control, test_treated, 
    n_test_control, n_test_treated, 
...
c(greedy_cates, greedy_bins) %<-% est_greedy(train_df, test_df, test_covs, test_control, test_treated,
                                             n_test_treated, 3, bart_fit)
c(miqp_cates, miqp_bins) %<-% est_MIQP_variance(train_df, test_df, test_treated, n_test_treated, train_covs, 
                                                test_covs, n_train, p, lambda=2, alpha=1, m=1)
...
me2 = abs(miqp_cates2 - HTE_test_treated)
