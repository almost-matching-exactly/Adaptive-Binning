...
# Simulations
########################################################
all_y_dgp <- list(y_dgp_bart)
all_p_vals <- list(5) # Need as many ps as dgps
...
for (dgp in 1:n_dgp) {
  ...
  for (sim in 1:nsims) {
    ...
    m_estimators <- c('Full Matching', 'Prognostic', 'CEM', 'Greedy',
    ...
    for (l in 1:n_test_treated){
      ...
      if(n_test_treated + n_test_control > 300){
        ...
        cands = c(ord[test_df$treated[ord]==1][2:100], ord[test_df$treated[ord]==0][1:100])
      }else{
        cands = (1:(n - n_train))[-i]
      }
