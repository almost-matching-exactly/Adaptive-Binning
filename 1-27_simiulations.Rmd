...
# Simulations
########################################################
all_y_dgp <- list(y_dgp_box)
all_p_vals <- list(2) # Need as many ps as dgps
...
for (dgp in 1:n_dgp) {
  ...
  for (sim in 1:nsims) {
    ...
    for (l in 1:n_test_treated){
      ...
      # cands = c(order(abs(fhat1[i] - fhat1[test_df$treated==1]) * gamma1/sd(fhat1) +
      #                abs(fhat0[i] - fhat0)[test_df$treated==1] * gamma1/sd(fhat0))[2:101],
      #           order(abs(fhat1[i] - fhat1[test_df$treated==0]) * gamma1/sd(fhat1) +
      #                abs(fhat0[i] - fhat0)[test_df$treated==0] * gamma1/sd(fhat0))[1:100])
      cands <- (1:nrow(test_covs))[-i]
      mip_pars =  setup_miqp_fhat(xi = as.numeric(test_covs[i, ]),
    ...
    fcates = format_CATEs(CATEs, true_CATE = HTE_test_treated,
                        estimators = c(m_estimators, c('Best CF', 'BART', 'BART of 1', 'BART of 2',  'MIP', 'MIP_both')))
...
x_dgp <- function(Z, p_rel, p_irrel) {
  ...
  for (i in 1:p_irrel) {
    x %<>%
      cbind(rbeta(n, shape1 = 1.5 + 5.5 * Z, shape2 = 6 - 4 * Z))
  }
  return(x)
}

y_dgp_FLAME <- function(X, Z, eps, U = 1, max_nonlinear = 5) {
  n <- nrow(X)
  
  s <- sample(c(-1, 1), size = 10, replace = TRUE)
  alpha <- rnorm(10, 10 *s, 1)
  baseline <- X %*% alpha
  
  beta <- rnorm(10, 1.5, 0.15)
  linear_treated <- X %*% beta * Z
...
for (sim in 1:nsims) {
  ...
  Y1 <- y_dgp(X, rep(1, n), eps)
  Y0 <- y_dgp(X, rep(0, n), eps)
  HTE <- Y1 - Y0
  Y <- Y1 * Z + Y0 * (1 - Z)
