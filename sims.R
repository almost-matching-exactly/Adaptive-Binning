...
matching_sim <- function(n_sims = 10, n_units = 100, p = 3, n_train = floor(n_units / 2),
  ...
  for (sim in 1:n_sims) {
    # HTE across both training and test
    c(df, HTE) %<-% simulate_data(n_units, p, n_train, X_dgp, e_dgp, y_dgp)  
    inputs <- estimator_inputs(df, n_train, n_units, black_box, cv = cv)
    ...
    HTE_test_treated <- HTE[intersect((n_train + 1):n_units, which(df$treated))]
